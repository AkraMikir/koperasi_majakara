...
@section('title', 'Edit Bank Registrasi')
...
<div class="space-y-6 max-w-2xl">
    ...
    <div>
        <div class="flex items-center space-x-2 text-sm text-gray-500 mb-2">
            ...
            <span class="text-gray-900 font-medium">Edit Bank</span>
        </div>
        <h1 class="text-3xl font-bold text-gray-900 font-display">Edit Bank Registrasi</h1>
        <p class="text-sm text-gray-500 mt-1">Perbarui informasi bank: <span class="font-semibold text-gray-700">{{ $data->nama_bank }}</span></p>
    </div>

    {{-- Form --}}
    <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
        <form method="POST" action="{{ route('admin.master-data.bank-regis.update', $data->id) }}" class="space-y-6">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <input type="text" id="kode_bank" name="kode_bank"
                        value="{{ old('kode_bank', $data->kode_bank) }}"
            ...
            <div class="bg-gray-50 rounded-xl p-4 border border-gray-100">
                <p class="text-xs text-gray-500">
                    ...
                    &nbsp;·&nbsp;
                    <span class="font-semibold">Ditambahkan:</span> {{ $data->created_at->format('d M Y, H:i') }}
                    &nbsp;·&nbsp;
                    <span class="font-semibold">Terakhir diubah:</span> {{ $data->updated_at->format('d M Y, H:i') }}
                </p>
            </div>
