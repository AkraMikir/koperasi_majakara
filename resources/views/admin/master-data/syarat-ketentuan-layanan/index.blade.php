...
@section('title', 'Master Syarat & Ketentuan Layanan')
...
<div class="space-y-6">
    ...
    <div class="flex items-center justify-between">
        <div>
            <div class="flex items-center space-x-2 text-sm text-gray-600 mb-2">
                ...
                <span>/</span>
                <span class="text-gray-900 font-medium">Syarat & Ketentuan Layanan</span>
            </div>
            <h1 class="text-3xl font-bold text-gray-900 font-display">Syarat & Ketentuan Layanan</h1>
            <p class="text-gray-600 mt-1 font-sans">Kelola isi syarat & ketentuan penggunaan pinjaman jangka panjang nasabah (tampil pada pop-up persetujuan nasabah)</p>
        </div>
    </div>
    ...
    <div class="bg-white rounded-2xl shadow-md border border-gray-100 p-6 md:p-8">
        @canCrudMasterData
        <form id="form-syarat-layanan" action="{{ route('admin.master-data.syarat-ketentuan-layanan.update') }}" method="POST" class="space-y-6">
            @csrf
            
            <input type="hidden" name="konten" id="syarat_layanan_input">

            <div class="space-y-2">
                <label class="block text-sm font-bold text-gray-700">Konten Syarat & Ketentuan</label>
            ...
            <div class="flex justify-end pt-4">
                <button type="submit" class="bg-[#674c1d] hover:bg-[#8b6f2f] text-white px-6 py-3 rounded-xl font-bold transition-all shadow-md hover:shadow-lg">
                    Simpan Perubahan Syarat Layanan
                </button>
            </div>
        </form>
        @else
        ...
        <div class="space-y-4">
            <div class="p-4 bg-yellow-50 text-yellow-800 rounded-xl text-sm border border-yellow-100">
                Mode lihat saja. Hanya Admin Utama (Owner) yang berwenang mengubah data syarat & ketentuan layanan.
            </div>
