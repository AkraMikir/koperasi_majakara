...
@section('title', 'Master Denda Pinjaman')
    ...
    <div class="flex items-center justify-between">
        <div>
            <div class="flex items-center space-x-2 text-sm text-gray-600 mb-2">
                ...
                <span>/</span>
                <span class="text-gray-900 font-medium">Master Denda Pinjaman</span>
            </div>
            <h1 class="text-3xl font-bold text-gray-900 font-display">Master Denda Pinjaman</h1>
            <p class="text-gray-600 mt-1">Kelola persentase denda keterlambatan pembayaran</p>
        </div>
        <a href="{{ route('admin.master-data.denda-pinjaman.create') }}" class="px-4 py-2 bg-gradient-to-r from-[#674c1d] to-[#8b6f2f] text-white rounded-lg hover:from-[#4a3514] hover:to-[#674c1d] transition-all shadow-md text-sm font-medium">
            + Tambah Data
        </a>
    </div>
            ...
            <table class="w-full">
                ...
                <tbody class="divide-y divide-gray-200">
                    ...
                    <tr class="hover:bg-gray-50 transition-colors">
                        ...
                        <td class="px-6 py-4">
                            <span class="text-sm font-bold text-[#8b6f2f]">{{ $item->denda_persen }}% per hari</span>
                        </td>
                        <td class="px-6 py-4">
                            <form action="{{ route('admin.master-data.denda-pinjaman.toggle-status', $item->id) }}" method="POST" class="inline">
                                @csrf
                        ...
                        <td class="px-6 py-4">
                            <div class="flex items-center justify-center space-x-2">
                                <a href="{{ route('admin.master-data.denda-pinjaman.edit', $item->id) }}" class="p-2 text-[#674c1d] hover:bg-[#674c1d]/10 rounded-lg">
                                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"></path>
                                    </svg>
                                </a>
                                <form action="{{ route('admin.master-data.denda-pinjaman.destroy', $item->id) }}" method="POST" onsubmit="return confirm('Yakin hapus data ini?')">
                                    @csrf
                                    @method('DELETE')
