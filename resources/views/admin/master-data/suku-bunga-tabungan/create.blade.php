...
@section('title', 'Tambah Suku Bunga Tabungan')
...
<div class="space-y-6">
    <div>
        ...
        <h1 class="text-3xl font-bold text-gray-900 font-display">Tambah Suku Bunga Tabungan</h1>
    </div>

    <div class="bg-white rounded-2xl shadow-md p-6 border border-gray-100">
        <form action="{{ route('admin.master-data.suku-bunga-tabungan.store') }}" method="POST" class="space-y-6">
            @csrf
            <div>
                <label class="block text-sm font-semibold text-gray-700 mb-2">Jenis Bunga *</label>
                <input type="text" name="jenis_bunga" value="{{ old('jenis_bunga') }}" required
                    ...
                    placeholder="Contoh: Bunga Tabungan Reguler">
                @error('jenis_bunga')
            ...
            <div>
                ...
                <div class="relative">
                    <input type="number" name="opsi_val" value="{{ old('opsi_val') }}" required min="0" max="100" step="0.0001"
                        class="w-full px-4 py-3 border-2 border-gray-200 rounded-xl focus:border-[#4a3514] focus:ring-2 focus:ring-[#4a3514]/20 outline-none">
                    <span class="absolute right-4 top-1/2 -translate-y-1/2 text-gray-500">%</span>
                </div>
                <p class="text-xs text-gray-500 mt-1">Contoh: 0.5 untuk 0,5% per periode</p>
            </div>

            <div class="flex items-center space-x-3 pt-4 border-t">
                <button type="submit" class="px-6 py-3 bg-linear-to-r from-[#4a3514] to-[#674c1d] text-white rounded-xl hover:from-[#674c1d] hover:to-[#8b6f2f] transition-all font-medium shadow-md">
                    Simpan Data
                </button>
