...
@section('title', 'Tambah Jenis Deposito')
...
<div class="space-y-6">
    <div>
        ...
        <h1 class="text-3xl font-bold text-gray-900 font-display">Tambah Jenis Deposito</h1>
    </div>

    <div class="bg-white rounded-2xl shadow-md p-6 border border-gray-100">
        <form action="{{ route('admin.master-data.jenis-deposito.store') }}" method="POST" class="space-y-6">
            @csrf
            
            <div>
                <label class="block text-sm font-semibold text-gray-700 mb-2">Nama Jenis *</label>
                <input type="text" name="nama_jenis" value="{{ old('nama_jenis') }}" required
                    ...
                    placeholder="Contoh: Deposito Reguler, Deposito Berjangka, dll">
                @error('nama_jenis')
            ...
            <div>
                ...
                <textarea name="deskripsi" rows="4"
                    class="w-full px-4 py-3 border-2 border-gray-200 rounded-xl focus:border-[#d4af37] focus:ring-2 focus:ring-[#d4af37]/20 outline-none"
                    placeholder="Deskripsi jenis deposito (opsional)">{{ old('deskripsi') }}</textarea>
            </div>
