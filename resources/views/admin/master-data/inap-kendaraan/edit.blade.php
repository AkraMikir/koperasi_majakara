...
<div class="max-w-3xl mx-auto">
    ...
    <div class="bg-white rounded-3xl shadow-xl shadow-gray-200/50 border border-gray-100 overflow-hidden">
        <form action="{{ route('admin.master-data.inap-kendaraan.update', $data->id) }}" method="POST" class="p-8 space-y-6" onsubmit="let input = document.getElementById('nominal_inap'); if(input) { input.value = input.value.replace(/[^0-9]/g, ''); }">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <input type="text" name="jenis_kendaraan" id="jenis_kendaraan" class="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl focus:ring-2 focus:ring-[#674c1d] focus:border-transparent transition-all font-bold text-gray-700" value="{{ old('jenis_kendaraan', $data->jenis_kendaraan) }}" required placeholder="Contoh: 100cc - 150cc">
                </div>
