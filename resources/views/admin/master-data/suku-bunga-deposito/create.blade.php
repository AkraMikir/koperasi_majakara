...
@section('title', 'Tambah Suku Bunga Deposito')
...
<div class="space-y-6">
    <div>
        ...
        <h1 class="text-3xl font-bold text-gray-900 font-display">Tambah Suku Bunga Deposito</h1>
    </div>

    <div class="bg-white rounded-2xl shadow-md p-6 border border-gray-100">
        <form action="{{ route('admin.master-data.suku-bunga-deposito.store') }}" method="POST" class="space-y-6">
            @csrf
            ...
            <div>
                ...
                <select name="tenor_id" required class="w-full px-4 py-3 border-2 border-gray-200 rounded-xl focus:border-[#d4af37] focus:ring-2 focus:ring-[#d4af37]/20 outline-none">
                    ...
                    @foreach($tenors as $tenor)
                        <option value="{{ $tenor->id }}" {{ old('tenor_id') == $tenor->id ? 'selected' : '' }}>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    ...
                    <div class="relative">
                        ...
                        <input type="text" name="min_nominal" id="min_nominal" value="{{ old('min_nominal') ? number_format(old('min_nominal'), 0, ',', '.') : '' }}" required oninput="formatCurrency(this)"
                ...
                <div>
                    ...
                    <div class="relative">
                        <span class="absolute left-4 top-1/2 -translate-y-1/2 text-gray-500">Rp</span>
                        <input type="text" name="max_nominal" id="max_nominal" value="{{ old('max_nominal') ? number_format(old('max_nominal'), 0, ',', '.') : '' }}" required oninput="formatCurrency(this)"
                            class="w-full pl-12 pr-4 py-3 border-2 border-gray-200 rounded-xl focus:border-[#d4af37] focus:ring-2 focus:ring-[#d4af37]/20 outline-none">
                    </div>
                </div>
            </div>
            ...
            <div>
                ...
                <div class="relative">
                    <input type="number" name="bunga" value="{{ old('bunga') }}" required min="0" max="100" step="0.0001"
