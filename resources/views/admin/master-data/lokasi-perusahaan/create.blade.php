...
@section('title', 'Tambah Lokasi Perusahaan')
...
<div class="space-y-6">
    <div>
        ...
        <h1 class="text-3xl font-bold text-gray-900 font-display">Tambah Lokasi Perusahaan</h1>
    </div>

    <div class="bg-white rounded-2xl shadow-md p-6 border border-gray-100">
        <form action="{{ route('admin.master-data.lokasi-perusahaan.store') }}" method="POST" class="space-y-6">
            @csrf
            ...
            <div>
                ...
                <input type="text" name="nama_lokasi" value="{{ old('nama_lokasi') }}" required
                    ...
                    placeholder="Contoh: Kantor Pusat Majakara">
                @error('nama_lokasi')
            ...
            <div>
                ...
                <select name="tipe_lokasi" required class="w-full px-4 py-3 border-2 border-gray-200 rounded-xl focus:border-[#674c1d] focus:ring-2 focus:ring-[#674c1d]/20 outline-none">
                    ...
                    <option value="kantor_pusat" {{ old('tipe_lokasi') == 'kantor_pusat' ? 'selected' : '' }}>Kantor Pusat</option>
                    <option value="kantor_cabang" {{ old('tipe_lokasi') == 'kantor_cabang' ? 'selected' : '' }}>Kantor Cabang</option>
                    <option value="agen" {{ old('tipe_lokasi') == 'agen' ? 'selected' : '' }}>Agen</option>
                </select>
            </div>
