...
@section('title', 'Edit Master Bunga Pinjaman')
...
<div class="space-y-6">
    ...
    <div class="flex items-center justify-between">
        <div>
            ...
            <h1 class="text-3xl font-bold text-gray-900 font-display">Edit Master Bunga Pinjaman</h1>
        </div>
    </div>

    <!-- Form -->
    <div class="bg-white rounded-2xl shadow-md p-6 border border-gray-100">
        <form action="{{ route('admin.master-data.bunga-pinjaman.update', $data->id) }}" method="POST" class="space-y-6">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <input type="number" name="durasi_min" value="{{ old('durasi_min', $data->durasi_min) }}" required min="1"
                ...
                <div>
                    ...
                    <input type="number" name="durasi_max" value="{{ old('durasi_max', $data->durasi_max) }}" required min="1"
                        class="w-full px-4 py-3 border-2 border-gray-200 rounded-xl focus:border-[#674c1d] focus:ring-2 focus:ring-[#674c1d]/20 outline-none transition-all">
                    @error('durasi_max')
                        <p class="text-xs text-red-600 mt-1">{{ $message }}</p>
                    @enderror
            ...
            <div>
                ...
                <input type="number" name="durasi_pilihan" value="{{ old('durasi_pilihan', $data->durasi_pilihan) }}" min="1" placeholder="Masukkan durasi eksak jika ingin membatasi pilihan (misal: 1 atau 3)"
                    class="w-full px-4 py-3 border-2 border-gray-200 rounded-xl focus:border-[#674c1d] focus:ring-2 focus:ring-[#674c1d]/20 outline-none transition-all">
            ...
            <div>
                ...
                <div class="relative">
                    <input type="number" name="bunga_persen" value="{{ old('bunga_persen', $data->bunga_persen) }}" required min="0" max="100" step="0.01"
                        class="w-full px-4 py-3 border-2 border-gray-200 rounded-xl focus:border-[#674c1d] focus:ring-2 focus:ring-[#674c1d]/20 outline-none transition-all">
            ...
            <div>
                ...
                <div class="relative">
                    <input type="number" name="bunga_flat_hari" value="{{ old('bunga_flat_hari', $data->bunga_flat_hari) }}" min="0" max="100" step="0.01" placeholder="Kosongkan untuk menghitung otomatis (Bunga % / 30)"
                        class="w-full px-4 py-3 border-2 border-gray-200 rounded-xl focus:border-[#674c1d] focus:ring-2 focus:ring-[#674c1d]/20 outline-none transition-all">
                    <span class="absolute right-4 top-1/2 -translate-y-1/2 text-gray-500 font-medium">%</span>
                </div>
            ...
            <div>
                ...
                <textarea name="keterangan" rows="3"
                    class="w-full px-4 py-3 border-2 border-gray-200 rounded-xl focus:border-[#674c1d] focus:ring-2 focus:ring-[#674c1d]/20 outline-none transition-all">{{ old('keterangan', $data->keterangan) }}</textarea>
                @error('keterangan')
                    <p class="text-xs text-red-600 mt-1">{{ $message }}</p>
                @enderror
            </div>
