...
@section('title', 'Tambah Master Denda Deposito')
...
<div class="space-y-6">
    <div class="flex items-center justify-between">
        <div>
            ...
            <h1 class="text-3xl font-bold text-gray-900 font-display">Tambah Master Denda Deposito</h1>
        </div>
    </div>

    <div class="bg-white rounded-2xl shadow-md p-6 border border-gray-100">
        <form action="{{ route('admin.master-data.denda-deposito.store') }}" method="POST" class="space-y-6">
            @csrf

            <div>
                <label class="block text-sm font-semibold text-gray-700 mb-2">Persentase Denda dari Nominal Awal (%) *</label>
                <div class="relative">
                    <input type="number" name="denda_persen" value="{{ old('denda_persen', 0.25) }}" required min="0" max="100" step="0.01"
