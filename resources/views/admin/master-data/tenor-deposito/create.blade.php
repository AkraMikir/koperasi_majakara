...
@section('title', 'Tambah Tenor Deposito')
...
<div class="space-y-6">
    <div>
        ...
        <h1 class="text-3xl font-bold text-gray-900 font-display">Tambah Tenor Deposito</h1>
    </div>

    <div class="bg-white rounded-2xl shadow-md p-6 border border-gray-100">
        <form action="{{ route('admin.master-data.tenor-deposito.store') }}" method="POST" class="space-y-6">
            @csrf
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    <label class="block text-sm font-semibold text-gray-700 mb-2">Tenor (Hari) *</label>
                    <input type="number" name="tenor_hari" value="{{ old('tenor_hari') }}" required min="1"
                    ...
                    <p class="text-xs text-gray-500 mt-1">Contoh: 30 untuk 30 hari</p>
                </div>

                <div>
                    <label class="block text-sm font-semibold text-gray-700 mb-2">Tenor (Bulan) *</label>
                    <input type="number" name="tenor_bulan" value="{{ old('tenor_bulan') }}" required min="1"
                        class="w-full px-4 py-3 border-2 border-gray-200 rounded-xl focus:border-[#d4af37] focus:ring-2 focus:ring-[#d4af37]/20 outline-none">
                    <p class="text-xs text-gray-500 mt-1">Contoh: 1 untuk 1 bulan</p>
                </div>
            </div>
