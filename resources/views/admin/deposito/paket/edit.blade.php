...
@section('title', 'Edit Paket Deposito')
...
<div class="space-y-6">
    ...
    <div class="flex items-center space-x-4 bg-white p-6 rounded-2xl border border-gray-100 shadow-xs">
        ...
        <div>
            <h1 class="text-2xl font-bold text-gray-900 font-display">Edit Paket Deposito</h1>
            <p class="text-sm text-gray-500 mt-1">Perbarui data paket deposito.</p>
        </div>
    </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div class="relative">
                        <select name="tenor_bulan" id="tenor_bulan" required
                            ...
                            @foreach($tenors as $tenor)
                                <option value="{{ $tenor->tenor_bulan }}" {{ old('tenor_bulan', $paket->tenor_bulan) == $tenor->tenor_bulan ? 'selected' : '' }}>{{ $tenor->tenor_bulan }} Bulan</option>
                            @endforeach
                        </select>
                ...
                <div>
                    ...
                    <div class="relative">
                        <input type="number" step="0.01" min="0" name="suku_bunga" id="suku_bunga" required
                            value="{{ old('suku_bunga', $paket->suku_bunga) }}"
                    ...
                    <div class="relative">
                        <select name="kategori_id" id="kategori_id"
                            ...
                            @foreach($kategoris as $kategori)
                                <option value="{{ $kategori->id }}" {{ old('kategori_id', $paket->kategori_id) == $kategori->id ? 'selected' : '' }}>{{ $kategori->nama_kategori }}</option>
                            @endforeach
                        </select>
                        <div class="pointer-events-none absolute inset-y-0 right-0 flex items-center px-3 text-gray-500">
                            <svg class="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"></path></svg>
                ...
                <div class="col-span-1 md:col-span-2">
                    ...
                    <textarea name="keterangan" id="keterangan" rows="3"
                        class="w-full rounded-xl border-gray-300 focus:border-[#8b6f2f] focus:ring focus:ring-[#8b6f2f]/20 transition-all placeholder-gray-400"
                        placeholder="Tambahkan informasi khusus mengenai paket ini (jika ada)">{{ old('keterangan', $paket->keterangan) }}</textarea>
                    @error('keterangan')
                        <p class="mt-1.5 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
