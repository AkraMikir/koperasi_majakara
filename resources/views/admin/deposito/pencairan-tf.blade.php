...
@section('title', 'Pencairan Deposito via Transfer')
...
<div class="p-6">
    ...
    <div class="flex items-center justify-between mb-6">
        <div>
            <h1 class="text-xl font-bold text-gray-800">Pencairan Deposito – Transfer (TF)</h1>
            <p class="text-sm text-gray-500 mt-0.5">Daftar permintaan pencairan dana ke rekening nasabah</p>
        </div>
    ...
    <div class="grid grid-cols-3 gap-4 mb-6">
        <div class="bg-amber-50 border border-amber-200 rounded-xl p-4 text-center">
            ...
            <p class="text-xs text-amber-600 font-semibold mt-1">Menunggu Proses</p>
        </div>
        ...
        <table class="w-full text-sm">
            <thead class="bg-gray-50 border-b border-gray-100">
                <tr>
                    ...
                    <th class="px-4 py-3 text-left text-xs font-bold text-[#674c1d] uppercase">No. Rekening</th>
            ...
            <tbody class="divide-y divide-gray-50">
                ...
                <tr class="hover:bg-gray-50 transition-colors">
                    ...
                    <td class="px-4 py-3 font-mono text-xs text-gray-600">
                        {{ $p->nasabah?->dataRek?->no_rek ?? '-' }}
                    </td>
                    <td class="px-4 py-3">
                        @if($p->status === 'pending')
                            <span class="px-2 py-1 bg-amber-100 text-amber-700 rounded-full text-xs font-bold">Pending</span>
                        @elseif($p->status === 'diproses')
                            <span class="px-2 py-1 bg-blue-100 text-blue-700 rounded-full text-xs font-bold">Diproses</span>
                        @elseif($p->status === 'selesai')
                            <span class="px-2 py-1 bg-green-100 text-green-700 rounded-full text-xs font-bold">Selesai</span>
                        @else
                            <span class="px-2 py-1 bg-red-100 text-red-700 rounded-full text-xs font-bold">{{ ucfirst($p->status) }}</span>
                        @endif
                    </td>
                    <td class="px-4 py-3">
                        @if($p->status === 'pending')
                        <a href="{{ route('admin.deposito.pencairan-tf.proses-form', $p->id) }}"
                ...
                <tr>
                    <td colspan="7" class="px-4 py-12 text-center text-gray-400">
                        <p class="text-sm font-semibold">Belum ada request pencairan TF</p>
                    </td>
                </tr>
                @endforelse
            </tbody>
