...
<div class="space-y-6">
    ...
    <div class="flex items-center justify-between">
        <div>
            ...
            <p class="text-gray-600 mt-1">Monitoring aliran dana operasional koperasi</p>
        </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
        {{-- Saldo Admin (Hanya tampil untuk Admin Operasional) --}}
        @if(auth()->user()->role === 'admin_operasional')
        <div class="bg-gradient-to-br {{ $isLowSaldo ? 'from-red-600 to-red-800' : 'from-green-600 to-green-800' }} rounded-2xl shadow-lg p-6 text-white col-span-1 md:col-span-1">
            <div class="flex items-center justify-between mb-4">
                ...
                <span class="text-white/60 text-xs font-medium uppercase tracking-wider">{{ $alertMessage }}</span>
            </div>
            <p class="text-white/80 text-sm mb-1">💵 Saku Cash Saya</p>
            <p class="text-3xl font-bold">Rp {{ number_format($saldoCash, 0, ',', '.') }}</p>
            @if($isLowSaldo)
            <p class="text-xs text-red-100 mt-2">Saldo di bawah Rp 500.000. Segera setor ke kantor!</p>
            @endif
        </div>

        <div class="bg-gradient-to-br from-blue-600 to-blue-800 rounded-2xl shadow-lg p-6 text-white col-span-1 md:col-span-1">
            <div class="flex items-center justify-between mb-4">
                <div class="w-12 h-12 bg-white/20 rounded-xl flex items-center justify-center">
                    <svg class="w-6 h-6 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <p class="text-white/80 text-sm mb-1">🏦 Rekening TF Saya</p>
            <p class="text-3xl font-bold">Rp {{ number_format($saldoTransfer, 0, ',', '.') }}</p>
            <p class="text-xs text-blue-100 mt-2">Saldo operasional non-cash</p>
        </div>
        @endif

        {{-- Saldo Owner --}}
        <div class="bg-gradient-to-br from-[#674c1d] to-[#4a3514] rounded-2xl shadow-lg p-6 text-white {{ auth()->user()->role !== 'admin_utama' ? 'hidden' : '' }}">
            <div class="flex items-center justify-between mb-4">
                <div class="w-12 h-12 bg-white/20 rounded-xl flex items-center justify-center">
                    <svg class="w-6 h-6 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <p class="text-white/80 text-sm mb-1">Saldo Owner</p>
            <p class="text-3xl font-bold">Rp {{ number_format($saldoOwner, 0, ',', '.') }}</p>
        </div>

        {{-- Alert Setoran Pending --}}
        @php $totalPending = $setoranPending->count(); @endphp
        <a href="{{ route('admin.petty-cash.setoran-approval.index') }}"
           class="bg-white rounded-2xl shadow-md p-6 border border-gray-100 hover:shadow-lg transition-all {{ $totalPending > 0 ? 'border-l-4 border-l-amber-500' : '' }}">
            <div class="flex items-center justify-between mb-4">
                ...
                @if($totalPending > 0)
                <span class="px-3 py-1 bg-amber-100 text-amber-700 rounded-full text-xs font-bold">{{ $totalPending }} Pending</span>
                @endif
            </div>
    ...
    <div class="bg-white rounded-2xl shadow-md p-6 border border-gray-100">
        <div class="flex items-center justify-between mb-6 pb-4 border-b border-gray-200">
            <div class="flex items-center space-x-3">
                ...
                <h2 class="text-lg font-bold text-[#674c1d] font-display">Saldo Petty Cash Per Admin</h2>
            </div>
        </div>
    ...
    <div class="bg-white rounded-2xl shadow-md p-6 border border-gray-100">
        <div class="flex items-center space-x-3 mb-6 pb-4 border-b border-gray-200">
            ...
            <h2 class="text-lg font-bold text-[#674c1d] font-display">Penerimaan vs Setoran (7 Hari)</h2>
        </div>
            ...
            <table class="w-full text-sm">
                <thead>
                    <tr class="border-b-2 border-[#674c1d]/20 bg-gradient-to-r from-[#674c1d]/5 to-[#8b6f2f]/5">
                        ...
                        <th class="px-4 py-3 text-right text-xs font-bold text-green-700 uppercase">Penerimaan (Owner → Admin)</th>
    ...
    <div class="bg-amber-50 border border-amber-200 rounded-2xl p-6">
        <div class="flex items-center gap-3 mb-4">
            ...
            <h2 class="text-lg font-bold text-amber-900">{{ $setoranPending->count() }} Setoran Menunggu Verifikasi</h2>
        </div>
        <div class="space-y-3">
