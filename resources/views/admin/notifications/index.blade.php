...
<div class="space-y-6">
    ...
    <div class="flex items-center justify-between">
        <div>
            <h1 class="text-3xl font-bold text-gray-900 font-display">Notifikasi</h1>
            <p class="text-gray-600 mt-1">Semua notifikasi dari pengajuan tabungan, pinjaman, dan aktivitas lainnya</p>
        </div>
        <div class="flex items-center space-x-3">
            @if($notifications->whereNull('read_at')->count() > 0)
                <form method="POST" action="{{ route('admin.notifications.mark-all-read') }}" class="inline">
                    @csrf
                    <button type="submit" class="px-4 py-2 bg-[#674c1d] text-white rounded-lg hover:bg-[#4a3514] transition-colors text-sm font-medium">
                        Tandai semua dibaca
                    </button>
                </form>
            @endif
            <a href="{{ route('admin.dashboard') }}" class="px-4 py-2 bg-white border border-gray-300 rounded-lg text-gray-700 hover:bg-gray-50 transition-colors text-sm font-medium">
                ← Dashboard
            </a>
        </div>
    </div>
    ...
    <div class="bg-white rounded-2xl shadow-md p-4 border border-gray-100">
        <form method="GET" action="{{ route('admin.notifications.index') }}" class="flex flex-wrap items-center gap-3">
    ...
    <div class="bg-white rounded-2xl shadow-md border border-gray-100 overflow-hidden">
        @forelse($notifications as $notif)
            <div class="border-b border-gray-100 last:border-0 {{ $notif->read_at ? 'bg-white' : 'bg-[#674c1d]/5' }}">
                <form method="POST" action="{{ route('admin.notifications.mark-read', $notif->id) }}" class="block">
                    @csrf
                    ...
                    <button type="submit" class="w-full text-left px-6 py-4 hover:bg-gray-50/50 transition-colors flex items-start gap-4">
                        <div class="shrink-0 w-10 h-10 rounded-xl flex items-center justify-center
                            ...
                            @elseif($notif->type === 'pendaftaran_nasabah') bg-indigo-100 text-indigo-700
                            ...
                            @elseif($notif->type === 'pendaftaran_nasabah')
        ...
        @empty
            <div class="px-6 py-16 text-center">
                ...
                <p class="text-sm text-gray-400 mt-1">Notifikasi dari pengajuan tabungan dan pinjaman akan muncul di sini.</p>
            </div>
        @endforelse
    </div>
