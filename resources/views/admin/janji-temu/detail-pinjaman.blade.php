                ...
                <div class="flex items-start gap-6 relative">
                    ...
                    <div class="space-y-4 flex-1">
                        <div>
                            ...
                            <p class="font-bold text-gray-900 text-lg">{{ $janjiTemu->nasabah->user->nama ?? 'N/A' }}</p>
                        </div>
                ...
                <div class="space-y-6">
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div class="p-4 bg-gray-50 rounded-xl border border-gray-100">
                            ...
                            <p class="font-bold text-gray-900 text-lg">Rp {{ number_format($janjiTemu->nominal ?? 0, 0, ',', '.') }}</p>
                        </div>
                    </div>
                    ...
                    <div class="p-4 bg-blue-50 rounded-xl border border-blue-100">
                        ...
                        <p class="font-medium text-gray-900">Durasi {{ $janjiTemu->pengajuan->durasi }} bulan, Jenis {{ ucfirst($janjiTemu->pengajuan->jenis ?? 'bulanan') }}</p>
                    </div>
                    @endif
                    ...
                    <div class="pt-4 border-t border-gray-100">
                        ...
                        <div class="grid grid-cols-2 sm:grid-cols-3 gap-4">
                            ...
                            <div class="aspect-square rounded-xl overflow-hidden border border-gray-200">
                                <img src="{{ asset('storage/' . $bukti->file_path) }}" class="w-full h-full object-cover" alt="Bukti">
                            </div>
                            @endforeach
                        </div>
                    </div>
        ...
        <div class="space-y-6 lg:sticky lg:top-6 lg:self-start">
            ...
            <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
                ...
                <p class="text-sm text-gray-500 mb-6">
                    Setelah pertemuan selesai dan uang diterima, proses janji temu ini untuk menyetujui dan mencairkan pinjaman (sistem setujui + cairkan sama seperti pengajuan).
                </p>
                ...
                @else
                <form action="{{ route('admin.pinjaman.janji-temu.proses-pinjaman', ['id' => $janjiTemu->id]) }}" method="post" enctype="multipart/form-data" accept-charset="UTF-8">
                    @csrf
                    <div class="space-y-4">
                        <div>
                            ...
                            <div class="relative">
                                ...
                                <input type="text" value="{{ number_format($janjiTemu->nominal ?? 0, 0, ',', '.') }}" class="w-full pl-10 pr-4 py-2.5 bg-gray-50 border border-gray-200 rounded-xl font-bold text-gray-900" readonly>
                            </div>
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-2">Tanggal Cair</label>
                            <input type="date" name="tgl_cair" value="{{ date('Y-m-d') }}" required class="w-full px-4 py-2.5 bg-gray-50 border border-gray-200 rounded-xl focus:ring-2 focus:ring-[#674c1d] focus:border-transparent outline-none">
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-2">Bukti Foto (Opsional)</label>
                            <input type="file" name="bukti_transfer" accept="image/*" class="w-full text-sm text-gray-500 file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:text-sm file:font-semibold file:bg-[#674c1d]/10 file:text-[#674c1d] hover:file:bg-[#674c1d]/20 transition-all">
                            <p class="text-xs text-gray-500 mt-1">Bisa upload foto uang / kwitansi / bukti penerimaan</p>
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-2">Keterangan Admin</label>
                            <textarea name="keterangan_admin" rows="3" class="w-full px-4 py-2.5 bg-gray-50 border border-gray-200 rounded-xl focus:ring-2 focus:ring-[#674c1d] focus:border-transparent outline-none" placeholder="Catatan tambahan..."></textarea>
                        </div>
                        <button type="submit" onclick="return confirm('Apakah Anda yakin uang sudah diterima? Pinjaman akan disetujui dan dicairkan (jadwal angsuran dibuat).')" class="w-full py-3 bg-[#674c1d] hover:bg-[#543d16] text-white font-bold rounded-xl shadow-lg shadow-[#674c1d]/20 transition-all">
            ...
            <div class="bg-[#674c1d] rounded-2xl shadow-lg p-6 text-white relative overflow-hidden">
                ...
                <div class="relative z-10">
                    ...
                    @if($janjiTemu->status == '1')
                    <span class="inline-flex px-4 py-1.5 rounded-full bg-yellow-400/20 border border-yellow-400/30 text-yellow-100 font-semibold text-sm">Menunggu Pertemuan</span>
                    <p class="mt-4 text-sm text-white/80">Proses janji temu dari halaman ini: isi keterangan dan klik Selesaikan & Cairkan.</p>
                    @elseif($janjiTemu->status == '2')
                    <span class="inline-flex px-4 py-1.5 rounded-full bg-green-400/20 border border-green-400/30 text-green-100 font-semibold text-sm">Selesai</span>
                    <p class="mt-4 text-sm text-white/80">Pertemuan telah selesai dan pinjaman telah dicairkan.</p>
                    @else
                    <span class="inline-flex px-4 py-1.5 rounded-full bg-red-400/20 border border-red-400/30 text-red-100 font-semibold text-sm">Dibatalkan</span>
                    @endif
                </div>
            </div>
        </div>
