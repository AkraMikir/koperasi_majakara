...
<div class="relative overflow-hidden rounded-2xl bg-gradient-to-br from-[#b91c1c] to-[#7f1d1d] p-6 mb-6 shadow-xl text-white">
    ...
    <div class="relative z-10 grid grid-cols-1 md:grid-cols-3 gap-6 divide-y md:divide-y-0 md:divide-x divide-white/10">
        <div class="space-y-1">
            <p class="text-red-200 text-[10px] uppercase font-semibold tracking-wider">Total Kewajiban Deposito</p>
            @php $totalKotorDeposito = $totalBersih + $totalPajak; @endphp
            <p class="text-2xl font-bold">Rp {{ number_format($totalKotorDeposito, 0, ',', '.') }}</p>
            <p class="text-red-300 text-xs">Bunga kotor aktif ({{ $depositoAktif }} deposito)</p>
            {{-- PPh Breakdown --}}
            <div class="flex items-center gap-2 mt-3 pt-3 border-t border-white/10">
                <div class="flex-1 bg-white/10 rounded-lg px-3 py-1.5">
                    <p class="text-[9px] text-red-200 font-semibold uppercase">80% ke Nasabah</p>
                    <p class="text-sm font-bold text-red-100">Rp {{ number_format($totalBersih, 0, ',', '.') }}</p>
                </div>
                <div class="flex-1 bg-rose-600/30 border border-rose-400/30 rounded-lg px-3 py-1.5">
                    <p class="text-[9px] text-rose-200 font-semibold uppercase">20% PPh</p>
                    <p class="text-sm font-bold text-rose-200">Rp {{ number_format($totalPajak, 0, ',', '.') }}</p>
                </div>
            </div>
        </div>
        ...
        <div class="space-y-1 pt-4 md:pt-0 md:pl-6">
            <p class="text-red-200 text-[10px] uppercase font-semibold tracking-wider">Estimasi Aktual</p>
            ...
            <a href="{{ route('admin.bunga.pajak.index') }}" class="inline-flex items-center gap-1.5 mt-3 text-[10px] font-semibold text-red-200 hover:text-white transition-colors">
...
<div class="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-5 mb-8">
    ...
    <div class="bg-white rounded-2xl shadow-sm border border-amber-100 p-5 ring-1 ring-amber-100">
        <div class="flex items-center justify-between gap-3 mb-3">
            ...
            <span class="text-[10px] font-semibold text-amber-700 bg-amber-50 px-2 py-0.5 rounded-full">Estimasi Aktual</span>
        </div>
        <p class="text-2xl font-bold text-amber-700">Rp {{ number_format($totalEstimasiAkrual, 0, ',', '.') }}</p>
        <p class="text-xs text-gray-400 mt-2">Bunga aktual deposito belum jatuh tempo H-7</p>
    </div>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        new Chart(document.getElementById('tenorChart').getContext('2d'), {
            ...
            data: {
                ...
                datasets: [{
                    data: @json($tenorData),
                    backgroundColor: ['#ef4444', '#f97316', '#eab308', '#8b5cf6', '#10b981', '#3b82f6'],
                    borderWidth: 0,
                    hoverOffset: 6
                }]
            },
