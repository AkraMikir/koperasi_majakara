...
@section('title', 'Edit Pembayaran Pajak - Koperasi Majakara')
...
<div class="space-y-6">
    ...
    <div class="flex items-center space-x-4 bg-white p-6 rounded-2xl border border-gray-100 shadow-xs">
        ...
        <div>
            <h1 class="text-2xl font-bold text-gray-900">Edit Catatan Pajak</h1>
            <p class="text-sm text-gray-500 mt-1">{{ $record->jenis_label }} — {{ $record->periode_label }}</p>
        </div>
    </div>

    {{-- Form --}}
    <div class="bg-white rounded-2xl border border-gray-100 shadow-xs overflow-hidden">
        <form method="POST" action="{{ route('admin.bunga.pajak.update', $record->id) }}" enctype="multipart/form-data" id="formPajak"
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                <div class="md:col-span-1">
                    ...
                    <select name="jenis_pajak" id="jenisPajak" required
                        ...
                        <option value="pph_pinjaman" {{ $record->jenis_pajak === 'pph_pinjaman' ? 'selected' : '' }}>PPh Pinjaman (15%)</option>
                        <option value="pph_gadai"    {{ $record->jenis_pajak === 'pph_gadai'    ? 'selected' : '' }}>PPh Gadai (15%)</option>
                        <option value="pph_deposito" {{ $record->jenis_pajak === 'pph_deposito' ? 'selected' : '' }}>PPh Deposito (20%)</option>
                    </select>
                ...
                <div>
                    ...
                    <select name="periode_bulan" id="periodeBulan" required
                        class="w-full rounded-xl border-gray-300 focus:border-[#8b6f2f] focus:ring focus:ring-[#8b6f2f]/20 transition-all">
                        @for($m = 1; $m <= 12; $m++)
                        <option value="{{ $m }}" {{ $record->periode_bulan == $m ? 'selected' : '' }}>
                ...
                <div>
                    ...
                    <select name="periode_tahun" id="periodeTahun" required
                        ...
                        <option value="{{ $y }}" {{ $record->periode_tahun == $y ? 'selected' : '' }}>{{ $y }}</option>
                        @endfor
                    </select>
                </div>
            </div>
            ...
            <div class="flex items-center gap-4">
                <button type="button" id="btnHitung"
                    ...
                    <span id="btnHitungText">Hitung Ulang</span>
                </button>
            ...
            <div>
                    ...
                    <div class="md:col-span-1">
                        ...
                        <div class="relative">
                            ...
                            <input type="text" id="jumlahKotorDisplay"
                                ...
                                required>
                            <input type="hidden" name="jumlah_kotor" id="jumlahKotor" value="{{ old('jumlah_kotor', $record->jumlah_kotor) }}">
                        </div>
                    ...
                    <div>
                        ...
                        <div class="relative">
                            <input type="number" name="tarif_persen" id="tarifPersen" step="0.01"
                                value="{{ old('tarif_persen', $record->tarif_persen) }}"
                    ...
                    <div>
                        ...
                        <div class="relative">
                            ...
                            <input type="text" id="jumlahPajakDisplay"
                                ...
                                readonly>
                            <input type="hidden" name="jumlah_pajak" id="jumlahPajak" value="{{ old('jumlah_pajak', $record->jumlah_pajak) }}">
                        </div>
                    </div>
                </div>
                {{-- Jumlah Bersih --}}
                <div class="mt-4 p-4 bg-green-50 border border-green-200 rounded-xl flex items-center justify-between">
                    ...
                    <div class="text-right">
                        ...
                        <input type="hidden" name="jumlah_bersih" id="jumlahBersih" value="{{ old('jumlah_bersih', $record->jumlah_bersih) }}">
                    </div>
                </div>
            </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        ...
                        <select name="status" required
                            class="w-full rounded-xl border-gray-300 focus:border-[#8b6f2f] focus:ring focus:ring-[#8b6f2f]/20 transition-all">
                            <option value="belum_bayar" {{ old('status', $record->status) === 'belum_bayar' ? 'selected' : '' }}>Belum Dibayar</option>
                            <option value="sudah_bayar" {{ old('status', $record->status) === 'sudah_bayar' ? 'selected' : '' }}>Sudah Dibayar</option>
                        </select>
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-2">Tanggal Bayar</label>
                        <input type="date" name="tanggal_bayar"
                            value="{{ old('tanggal_bayar', $record->tanggal_bayar?->format('Y-m-d')) }}"
                            class="w-full rounded-xl border-gray-300 focus:border-[#8b6f2f] focus:ring focus:ring-[#8b6f2f]/20 transition-all">
                    </div>
                    <div class="md:col-span-2">
                        ...
                        <textarea name="keterangan" rows="3"
                            ...
                            class="w-full rounded-xl border-gray-300 focus:border-[#8b6f2f] focus:ring focus:ring-[#8b6f2f]/20 transition-all placeholder-gray-400 resize-none">{{ old('keterangan', $record->keterangan) }}</textarea>
                    </div>
                </div>
            </div>
            ...
            <div>
                ...
                <div class="flex items-center gap-3 mb-3 p-3 bg-blue-50 border border-blue-200 rounded-xl max-w-md">
                    ...
                    <div class="flex-1 min-w-0">
                        <p class="text-xs text-blue-700 font-medium truncate">{{ basename($record->bukti_bayar) }}</p>
                        <a href="{{ Storage::url($record->bukti_bayar) }}" target="_blank" class="text-xs text-blue-500 hover:underline">Lihat file saat ini</a>
                    </div>
                </div>
                @endif
                <div class="border-2 border-dashed border-gray-200 rounded-xl p-8 text-center hover:border-[#8b6f2f]/50 transition-colors cursor-pointer"
                    ...
                    <p class="text-sm font-medium text-gray-500" id="fileLabel">{{ $record->bukti_bayar ? 'Ganti file (opsional)' : 'Klik untuk upload foto atau PDF bukti setor' }}</p>
