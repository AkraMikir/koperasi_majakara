        ...
        <div class="bg-white rounded-2xl shadow-md p-6 border border-gray-100">
            <div class="flex items-center justify-between mb-6 pb-4 border-b border-gray-200">
                <div class="flex items-center space-x-3">
                    ...
                    <h2 class="text-lg font-bold text-primary font-display">Pembayaran Terbaru</h2>
                </div>
                <a href="{{ route('admin.pinjaman.pembayaran') }}" class="text-sm text-[#674c1d] hover:underline font-medium">
                    Lihat Semua →
                </a>
            </div>
            ...
            <div class="space-y-4">
                ...
                <div class="flex items-center justify-between p-4 bg-gray-50 rounded-xl hover:bg-gray-100 transition-colors border border-gray-200">
                    <div class="flex items-center space-x-4 flex-1">
                        ...
                        <div class="flex-1">
                            <h3 class="font-semibold text-gray-900">{{ $bayar->nasabah->user->nama ?? 'N/A' }}</h3>
                            <p class="text-sm text-gray-600">Rp {{ number_format($bayar->nominal, 0, ',', '.') }}</p>
                            <p class="text-xs text-gray-500 mt-1">{{ $bayar->created_at->format('d M Y, H:i') }}</p>
                        </div>
                    </div>
                    <div class="flex items-center space-x-2">
                        @php
                            $statusLabels = ['1' => 'Pending', '2' => 'Ditolak', '3' => 'Disetujui', '4' => 'Terlaksana'];
                            $statusClass = ['1' => 'bg-yellow-100 text-yellow-700', '2' => 'bg-red-100 text-red-700', '3' => 'bg-blue-100 text-blue-700', '4' => 'bg-green-100 text-green-700'];
                            $s = $bayar->status ?? '1';
                        @endphp
                        <span class="px-3 py-1 {{ $statusClass[$s] ?? $statusClass['1'] }} rounded-full text-xs font-semibold">{{ $statusLabels[$s] ?? 'Pending' }}</span>
                        <a href="{{ route('admin.pinjaman.detail-pembayaran', $bayar->id) }}" class="p-2 text-[#674c1d] hover:bg-[#674c1d]/10 rounded-lg transition-colors">
                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7"></path>
                            </svg>
                        </a>
