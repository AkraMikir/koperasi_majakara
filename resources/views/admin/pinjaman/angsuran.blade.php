                    ...
                    <tr class="border-b border-gray-100 hover:bg-gray-50/50">
                        ...
                        <td class="px-4 py-3 text-center align-middle">
                            <div class="flex flex-col gap-1.5 items-center">
                                @foreach($tempos as $t)
                                <div class="flex flex-col sm:flex-row items-stretch sm:items-center gap-1 w-full justify-center">
                                    <a href="{{ route('admin.pinjaman.detail-angsuran', $t->id) }}?jenis={{ $jenis }}"
                                        class="inline-flex items-center justify-center px-2.5 py-1.5 bg-linear-to-r from-[#674c1d] to-[#8b6f2f] text-white rounded-lg hover:from-[#4a3514] hover:to-[#674c1d] transition-all text-xs font-medium shadow-sm w-full sm:w-auto text-center">
                                        Detail #{{ $t->no_urut }}
                                    </a>
                                    @if($t->status_bayar !== 'lunas')
                                        <button type="button"
                                            onclick="kirimPengingatWa('{{ $jenis }}', '{{ $t->id }}', this)"
                                            class="inline-flex items-center justify-center gap-1.5 p-1.5 bg-emerald-600 text-white rounded-lg hover:bg-emerald-700 transition-all text-xs font-medium shadow-sm hover:scale-105 w-full sm:w-auto"
                                            ...
                                            <span class="sm:hidden ml-1">Kirim WA</span>
                                        </button>
                                    @endif
                                </div>
                                @endforeach
