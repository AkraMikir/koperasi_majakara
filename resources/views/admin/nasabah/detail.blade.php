...
<div class="space-y-6">
    ...
    <!-- Reset PIN Nasabah - Admin Utama & Admin Operasional -->
    @if(auth()->check() && app(\App\Services\AdminPermissionService::class)->canResetNasabahPin(auth()->user()))
    ...
    <!-- Reset Password Nasabah - Admin Utama & Admin Operasional -->
    @if(auth()->check() && app(\App\Services\AdminPermissionService::class)->canResetNasabahPassword(auth()->user()))
    <div class="bg-white rounded-2xl shadow-lg p-6 border-2 border-orange-100">
        <div class="flex items-center gap-4 mb-6">
            <div class="w-14 h-14 bg-linear-to-br from-orange-500 to-orange-600 rounded-xl flex items-center justify-center shadow-md">
                <svg class="w-7 h-7 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 7a2 2 0 012 2m4 0a6 6 0 01-7.743 5.743L11 17H9v2H7v2H4a1 1 0 01-1-1v-2.586a1 1 0 01.293-.707l5.964-5.964A6 6 0 1121 9z"></path>
                </svg>
            </div>
            <div class="flex-1">
                <h2 class="text-xl font-bold text-gray-900 font-display">Reset Password Nasabah</h2>
                <p class="text-sm text-gray-600 mt-1">Untuk nasabah yang lupa password login</p>
            </div>
        </div>
        ...
        <div class="bg-yellow-50 border-l-4 border-yellow-500 p-4 mb-6 rounded-r-xl">
            <div class="flex items-start gap-3">
                ...
                <div class="text-sm">
                    <p class="font-semibold text-yellow-800 mb-1">Penting:</p>
                    <ul class="list-disc list-inside space-y-1 text-yellow-700">
                        <li>Pastikan nasabah benar-benar meminta reset password</li>
                        <li>Verifikasi identitas nasabah via WhatsApp/Telepon</li>
                        <li>Kirimkan password baru ke nasabah via WhatsApp dengan aman</li>
                        <li>Jangan share password di channel publik</li>
                    </ul>
                </div>
            </div>
        </div>

        <form id="form-reset-password" action="{{ route('admin.nasabah.reset-password', $nasabah->id) }}" method="POST" class="space-y-4">
            @csrf

            <div>
                <label class="block text-sm font-semibold text-gray-700 mb-2">Password Baru (Min. 8 Karakter) *</label>
                <div class="flex gap-3">
                    <input type="text" name="password_baru" id="password_baru" minlength="8" required
                        class="flex-1 px-4 py-3 border-2 border-gray-300 rounded-xl focus:border-orange-500 focus:ring-2 focus:ring-orange-500/20 outline-none font-mono tracking-widest"
                        placeholder="Password baru nasabah">
                    <button type="button" onclick="generateRandomPassword()" class="px-6 py-3 bg-linear-to-r from-blue-600 to-blue-700 text-white rounded-xl font-semibold hover:from-blue-700 hover:to-blue-800 transition-all shadow-md flex items-center gap-2">
                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15"></path>
                        </svg>
                        Generate Random
                ...
                <p class="text-xs text-gray-500 mt-2">Klik "Generate Random" untuk membuat password acak</p>
            </div>

            <div>
                <label class="block text-sm font-semibold text-gray-700 mb-2">Konfirmasi Password Baru *</label>
                <input type="text" name="password_baru_confirmation" id="password_baru_confirmation" minlength="8" required
                    class="w-full px-4 py-3 border-2 border-gray-300 rounded-xl focus:border-orange-500 focus:ring-2 focus:ring-orange-500/20 outline-none font-mono tracking-widest"
            ...
            <div class="bg-blue-50 border border-blue-200 rounded-xl p-4">
                <div class="flex gap-3">
                    ...
                    <div class="text-sm text-blue-800">
                        <p class="font-semibold mb-1">Langkah Selanjutnya:</p>
                        <ol class="list-decimal list-inside space-y-1">
                            <li>Klik "Reset Password" untuk menyimpan password baru</li>
                            <li>Copy password yang sudah dibuat</li>
                            <li>Kirim ke nasabah via WhatsApp: <strong class="font-mono">{{ $nasabah->user->nomor_hp }}</strong></li>
                            <li>Pastikan nasabah menerima dan mencatat password baru</li>
                        </ol>
                    </div>
                </div>
            </div>
            ...
            <div class="flex gap-3">
                <button type="submit" onclick="return confirm('Yakin reset password nasabah ini? Password lama akan diganti.')" 
                    class="flex-1 px-6 py-4 bg-linear-to-r from-orange-500 to-orange-600 text-white rounded-xl font-bold shadow-lg hover:shadow-xl transition-all transform hover:scale-[1.01] flex items-center justify-center gap-2">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z"></path>
                    </svg>
...
function generateRandomPassword() {
    ...
    document.getElementById('password_baru').value = password;
    document.getElementById('password_baru_confirmation').value = password;
    ...
    navigator.clipboard.writeText(password).then(() => {
        ...
        toast.innerHTML = `
            <div class="flex items-center gap-2">
                ...
                <span class="font-semibold">Password disalin ke clipboard!</span>
            </div>
        `;
        document.body.appendChild(toast);
        setTimeout(() => toast.remove(), 3000);
