...
<div class="space-y-8 max-w-5xl mx-auto">
    ...
    <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
        <div class="p-8">
            <form action="{{ route('admin.gadai_baru.store') }}" method="POST" enctype="multipart/form-data" class="space-y-8" onsubmit="document.getElementById('btnSubmit').disabled=true; document.getElementById('btnSubmit').innerHTML='Memproses...';">
                @csrf
                ...
                <div class="space-y-4">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            <label class="block text-sm font-bold text-gray-700 mb-2">Nasabah Pemohon <span class="text-red-500">*</span></label>
                            <select name="nasabah_id" class="w-full border-gray-200 rounded-xl bg-gray-50 focus:ring-[#674c1d] focus:border-[#674c1d] select2" required>
                                <option value="">Pilih Nasabah...</option>
                                @foreach($nasabahs as $n)
                                    <option value="{{ $n->id }}">{{ $n->user->nama ?? 'Tanpa Nama' }} ({{ $n->user->nomor_hp ?? 'Tanpa HP' }})</option>
                                @endforeach
                            </select>
                        </div>

                        <div>
                            <label class="block text-sm font-bold text-gray-700 mb-2">Cabang Penyimpanan <span class="text-red-500">*</span></label>
                            <select name="lokasi_id" class="w-full border-gray-200 rounded-xl bg-gray-50 focus:ring-[#674c1d] focus:border-[#674c1d]" required>
                ...
                <div class="space-y-4">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            ...
                            <select name="kategori_id" id="kategori_id" class="w-full border-gray-200 rounded-xl bg-gray-50 focus:ring-[#674c1d] focus:border-[#674c1d]" required>
                        ...
                        <div>
                            <label class="block text-sm font-bold text-gray-700 mb-2">Item Barang <span class="text-red-500">*</span></label>
                            <select name="item_id" id="item_id" class="w-full border-gray-200 rounded-xl bg-gray-50 focus:ring-[#674c1d] focus:border-[#674c1d]" required>
                                <option value="">Pilih Item</option>
                                @foreach($itemList as $item)
                                    <option value="{{ $item->id }}" data-kategori="{{ $item->kategori_id }}" data-max="{{ $item->nominal_high }}" class="item-option hidden">{{ $item->head_1 }} @if($item->head_2)({{ $item->head_2 }})@endif</option>
                                @endforeach
                            </select>
                            <p id="taksiran_info" class="text-xs text-amber-600 font-bold mt-2 hidden">Max Taksiran: Rp <span id="max_taksiran_text">0</span></p>
                        </div>

                        <div>
                            <label class="block text-sm font-bold text-gray-700 mb-2">Nominal Deal (Rp) <span class="text-red-500">*</span></label>
                            <div class="relative">
                                <div class="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
                                    <span class="text-gray-500 font-medium">Rp</span>
                                </div>
                                <input type="number" name="nominal_deal" id="nominal_deal" class="w-full pl-10 border-gray-200 rounded-xl bg-gray-50 focus:ring-[#674c1d] focus:border-[#674c1d] font-bold text-gray-900" required placeholder="0">
                            </div>
                            <p id="error_nominal" class="text-xs text-red-500 font-bold mt-2 hidden flex items-center gap-1">
                        ...
                        <div>
                            <label class="block text-sm font-bold text-gray-700 mb-2">Metode Pencairan <span class="text-red-500">*</span></label>
                            <select name="metode_pencairan" class="w-full border-gray-200 rounded-xl bg-gray-50 focus:ring-[#674c1d] focus:border-[#674c1d]" required>
                ...
                <div class="space-y-4">
                    <div class="flex items-center gap-2 pb-2 border-b border-gray-100">
                        ...
                        <h3 class="text-lg font-bold text-gray-800">Bukti Fisik Barang</h3>
                    </div>

                    <div>
                        <div class="flex items-center justify-between mb-2">
                            <label class="block text-sm font-bold text-gray-700">Upload Foto Bukti <span class="text-red-500">*</span></label>
                            <button type="button" id="add_file_btn" class="flex items-center gap-1 px-3 py-1.5 bg-emerald-100 text-emerald-700 hover:bg-emerald-200 rounded-lg text-xs font-bold transition-colors">
                        ...
                        <div id="file_upload_container" class="space-y-3">
                            <div class="flex items-center gap-2 file-input-group">
                                ...
                                <button type="button" class="w-11 h-11 flex items-center justify-center bg-red-50 text-red-500 rounded-xl border border-red-100 hover:bg-red-100 transition-colors cursor-not-allowed opacity-50" disabled>
                                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"></path></svg>
                                </button>
                            </div>
                        </div>
                        
                        <p class="text-xs text-gray-500 mt-2">Format yang didukung: JPG, JPEG, PNG. Maksimal 2MB per file. Harap fotokan barang dari berbagai sisi beserta dokumen kelengkapannya.</p>
                    </div>
                </div>
                ...
                <div class="bg-[#8b6f2f]/10 border border-[#8b6f2f]/20 rounded-xl p-5 flex items-start gap-4">
                    <div class="w-10 h-10 rounded-full bg-white flex items-center justify-center shrink-0 shadow-sm">
                        <svg class="w-6 h-6 text-[#8b6f2f]" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z"></path></svg>
                    </div>
                    <div>
                        <h4 class="font-bold text-gray-900">Perhatian Sebelum Submit</h4>
                        <p class="text-sm text-gray-600 mt-1 leading-relaxed">
                            Setelah menekan tombol simpan, dana <strong>otomatis akan memotong Saldo Cash Petty Cash</strong> Anda dan dicatat sebagai pengeluaran kas. Sistem juga akan mencari <strong>Slot Kosong Terbawah</strong> secara otomatis di tabel grid penyimpanan sesuai kategori barang.
                        </p>
                    </div>
                </div>
