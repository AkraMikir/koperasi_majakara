<div class="overflow-x-auto flex-1">
    <table class="w-full">
        <thead>
            <tr class="bg-linear-to-r from-[#674c1d] to-[#8b6f2f] text-white">
                <th class="px-6 py-3 text-left text-xs font-bold uppercase">ID</th>
                <th class="px-6 py-3 text-left text-xs font-bold uppercase">Jumlah</th>
                <th class="px-6 py-3 text-left text-xs font-bold uppercase">Jenis</th>
                <th class="px-6 py-3 text-left text-xs font-bold uppercase">Aksi</th>
            </tr>
        </thead>
        <tbody class="divide-y divide-gray-100">
            @forelse($transTabungans as $trans)
                <tr class="hover:bg-gray-50/50 transition-colors">
                    <td class="px-6 py-4">
                        <p class="text-sm font-mono text-gray-600">{{ $trans->id }}</p>
                        <p class="text-[10px] text-gray-400 mt-1">{{ $trans->tgl_transaksi->format('d M Y') }}</p>
                    </td>
                    <td class="px-6 py-4">
                        <p class="text-sm font-bold {{ $trans->jenis === 'setoran' ? 'text-green-600' : 'text-red-600' }}">
                            {{ $trans->jenis === 'setoran' ? '+' : '-' }}Rp {{ number_format(abs((float) $trans->nominal), 0, ',', '.') }}
                        </p>
                    </td>
                    <td class="px-6 py-4">
                        <span class="px-2 py-1 rounded-full text-[10px] font-bold uppercase 
                            {{ $trans->jenis === 'setoran' ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700' }}">
                            {{ $trans->jenis }}
                        </span>
                    </td>
                    <td class="px-6 py-4">
                        <a href="{{ route('nasabah.tabungan.detail-transaksi', $trans->id) }}" class="text-[#674c1d] hover:text-[#8b6f2f] text-xs font-semibold flex items-center gap-1 group border-none bg-transparent">
                            Detail
                            <svg class="w-3 h-3 group-hover:translate-x-1 transition-transform" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7"></path>
                            </svg>
                        </a>
                    </td>
                </tr>
            @empty
                <tr>
                    <td colspan="4" class="px-6 py-12 text-center text-gray-400 text-sm italic">Belum ada transaksi</td>
                </tr>
            @endforelse
        </tbody>
    </table>
</div>
@if($transTabungans->hasPages())
    <div class="px-6 py-4 border-t border-gray-100 bg-gray-50/50 pagination-container">
        {{ $transTabungans->links() }}
    </div>
@endif
