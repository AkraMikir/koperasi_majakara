<div class="overflow-x-auto flex-1">
    <table class="w-full">
        <thead>
            <tr class="bg-linear-to-r from-[#674c1d] to-[#8b6f2f] text-white">
        ...
        <tbody class="divide-y divide-gray-100">
            @forelse($pengajuanSetors as $item)
                @php
                    $statusConfig = [
                ...
                <tr class="hover:bg-gray-50/50 transition-colors">
                    ...
                    <td class="px-6 py-4">
                        <p class="text-sm font-bold text-[#674c1d]">Rp {{ number_format($item->nominal, 0, ',', '.') }}</p>
                    </td>
                    <td class="px-6 py-4">
                    ...
                    <td class="px-6 py-4">
                        <a href="{{ route('nasabah.tabungan.detail-pengajuan-setor', $item->id) }}" class="text-[#674c1d] hover:text-[#8b6f2f] text-xs font-semibold flex items-center gap-1 group border-none bg-transparent">
                            Detail
            ...
            @empty
                <tr>
                    <td colspan="4" class="px-6 py-12 text-center text-gray-400 text-sm italic">Belum ada pengajuan setor</td>
                </tr>
            @endforelse
        </tbody>
    </table>
</div>
@if($pengajuanSetors->hasPages())
    <div class="px-6 py-4 border-t border-gray-100 bg-gray-50/50 pagination-container">
        {{ $pengajuanSetors->links() }}
    </div>
@endif
