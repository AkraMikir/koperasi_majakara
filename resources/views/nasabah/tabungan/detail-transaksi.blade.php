...
<div class="w-full pb-6">
    ...
    <div class="mx-4 mb-6 space-y-6">
        ...
        <!-- Bukti Transaksi Manual (dari Admin) -->
        <!-- Bukti Transaksi Manual / Janji Temu (dari Admin) -->
        @php
            $hasManualProof = $transaksi->buktiFoto->count() > 0;
            $hasJanjiTemuProof = $transaksi->janjiTemuTabungan && $transaksi->janjiTemuTabungan->buktiFoto->count() > 0;
        @endphp
        ...
        <div class="bg-white rounded-2xl shadow-lg p-6 border border-gray-100">
            <div class="flex items-center gap-3 mb-4">
                ...
                <h2 class="text-lg font-bold text-[#674c1d] font-display">Bukti Transaksi</h2>
            </div>
            <p class="text-sm text-gray-600 mb-4">Bukti transaksi yang di-upload oleh admin</p>
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                {{-- Direct Transaction Proofs --}}
                @foreach($transaksi->buktiFoto as $bukti)
                <div class="border border-gray-200 rounded-xl overflow-hidden hover:shadow-lg transition-shadow cursor-pointer" 
                    onclick="showPhotoPreview('{{ asset('storage/' . $bukti->file_path) }}', 'Bukti transaksi manual - {{ ($transaksi->jenis ?? '') === 'setoran' ? 'Setoran' : 'Penarikan' }}')">
                    <img src="{{ asset('storage/' . $bukti->file_path) }}" alt="Bukti Transaksi" class="w-full h-48 object-cover">
                    <div class="p-4 bg-gray-50">
                        <p class="text-xs text-gray-500 mb-2">Upload: {{ $bukti->created_at->format('d M Y, H:i') }}</p>
                        <p class="text-sm text-gray-600 flex items-center gap-2">
                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0zM10 7v3m0 0v3m0-3h3m-3 0H7"></path>
                            </svg>
                ...
                {{-- Also show photos from related Janji Temu if any --}}
                @if($hasJanjiTemuProof)
                    @foreach($transaksi->janjiTemuTabungan->buktiFoto as $bukti)
                    <div class="border border-gray-200 rounded-xl overflow-hidden hover:shadow-lg transition-shadow cursor-pointer" 
                        onclick="showPhotoPreview('{{ asset('storage/' . $bukti->file_path) }}', 'Bukti Janji Temu - {{ ($transaksi->jenis ?? '') === 'setoran' ? 'Setoran' : 'Penarikan' }}')">
                        <img src="{{ asset('storage/' . $bukti->file_path) }}" alt="Bukti Janji Temu" class="w-full h-48 object-cover">
                        <div class="p-4 bg-gray-50">
                            <p class="text-xs text-gray-500 mb-2">Upload: {{ $bukti->created_at->format('d M Y, H:i') }}</p>
                            <p class="text-sm text-gray-600 flex items-center gap-2">
                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        @elseif($transaksi->pengajuanTarik && $transaksi->pengajuanTarik->foto_bukti_tf_admin)
        ...
        <div class="bg-white rounded-2xl shadow-lg p-6 border border-gray-100">
            <div class="flex items-center gap-3 mb-4">
                ...
                <h2 class="text-lg font-bold text-[#674c1d] font-display">Bukti Transfer Admin</h2>
            </div>
            <p class="text-sm text-gray-600 mb-4">Bukti transfer yang dikirim oleh admin koperasi</p>
            <div class="max-w-md border border-gray-200 rounded-xl overflow-hidden hover:shadow-lg transition-shadow cursor-pointer" 
                onclick="showPhotoPreview('{{ asset('storage/' . $transaksi->pengajuanTarik->foto_bukti_tf_admin) }}', 'Bukti Transfer Admin')">
                <img src="{{ asset('storage/' . $transaksi->pengajuanTarik->foto_bukti_tf_admin) }}" alt="Bukti Transfer" class="w-full h-auto">
