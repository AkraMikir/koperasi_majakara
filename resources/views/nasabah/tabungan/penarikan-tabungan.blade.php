        ...
        <div class="bg-gradient-to-br from-[#674c1d] via-[#8b6f2f] to-[#d4af37] rounded-3xl shadow-2xl p-6 border-2 border-[#d4af37]/30">
            <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-2">
                <div>
                    ...
                    <p class="text-2xl sm:text-4xl font-bold text-white font-display">Rp {{ number_format($tabunganInfo->saldo ?? 0, 0, ',', '.') }}</p>
                </div>
                ...
                <a href="{{ route('nasabah.tabungan.detail-transaksi', $riwayat->id) }}" class="block p-4 bg-gray-50 rounded-xl border border-gray-200 hover:border-red-300 hover:shadow-md transition-all">
                    <div class="flex items-center justify-between">
                        ...
                        <div class="text-right">
                            ...
                            <p class="text-xs text-gray-500 font-mono truncate">{{ $riwayat->id_transaksi ?? str_pad($riwayat->id, 5, '0', STR_PAD_LEFT) }}</p>
                        </div>
                    </div>
...
<div id="pin-modal" class="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 hidden items-center justify-center p-4">
    <div class="bg-white rounded-2xl shadow-2xl max-w-md w-full p-6 transform transition-all">
        ...
        <div class="flex gap-3">
            ...
            <button onclick="verifyAndSubmit()" id="btn-verify-submit" class="flex-1 px-4 py-3 bg-gradient-to-r from-[#674c1d] to-[#8b6f2f] text-white rounded-xl font-semibold hover:from-[#4a3514] hover:to-[#674c1d] transition-all">
                Verifikasi
            </button>
        </div>
    </div>
