        ...
        <div class="bg-linear-to-br from-[#8b6f2f] via-[#d4af37] to-[#674c1d] rounded-3xl shadow-2xl p-6 border-2 border-[#d4af37]/30">
            <div class="flex items-center justify-between">
                <div class="flex items-center gap-4">
                    ...
                    <div>
                        <h1 class="text-2xl font-bold text-white font-display mb-1">Status Penarikan</h1>
        ...
        <div class="flex items-center gap-3 overflow-x-auto pb-4 no-scrollbar">
            @php
                $filters = [
                    ...
                    '3' => ['label' => 'Ditolak', 'icon' => 'M10 14l2-2m0 0l2-2m-2 2l-2-2m2 2l2 2m7-2a9 9 0 11-18 0 9 9 0 0118 0z'],
                ];
            @endphp
            @foreach($filters as $val => $f)
                <a href="{{ route('nasabah.tabungan.status-pengajuan-tarik', ['status' => $val]) }}"
                    ...
                    {{ ($currentStatus ?? null) == $val 
                        ? 'bg-[#8b6f2f] text-white border-[#8b6f2f] shadow-xl shadow-[#8b6f2f]/20' 
                        : 'bg-white text-gray-500 border-gray-100 hover:border-[#8b6f2f]/30 hover:text-[#8b6f2f] shadow-sm' }}">
        ...
        <div class="space-y-4">
                ...
                <div class="p-6">
                    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
                        <div class="flex items-center gap-3">
                            <div class="w-12 h-12 bg-gray-50 rounded-2xl flex items-center justify-center text-[#8b6f2f]">
                            ...
                            <div>
                                ...
                                <div class="flex items-center gap-2">
                                    <p class="text-lg font-extrabold text-[#8b6f2f]">#{{ $item->id }}</p>
                                    @if($item->metode_transfer == 'transfer')
                                    ...
                                    @else
                                    <span class="px-2 py-0.5 bg-purple-50 text-purple-600 rounded-md text-[10px] font-bold uppercase tracking-tighter">Via Tunai</span>
                                    @endif
                                </div>
                            </div>
                        </div>
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-6 mb-6">
                        ...
                        <div class="space-y-1">
                            ...
                            <p class="font-extrabold text-[#8b6f2f] text-xl">Rp {{ number_format($item->nominal ?? 0, 0, ',', '.') }}</p>
                        </div>
                        
                        @if($item->metode_transfer == 'transfer')
                        <div class="space-y-1">
                            <p class="text-xs font-bold text-gray-400 uppercase">Bank Tujuan</p>
                            <p class="font-bold text-gray-900">{{ $item->nama_bank ?? '-' }}</p>
                        </div>
                        <div class="space-y-1">
                            <p class="text-xs font-bold text-gray-400 uppercase">No. Rekening</p>
                            <p class="font-bold text-gray-900 font-mono">{{ $item->no_rekening ?? '-' }}</p>
                        </div>
                        @else
                        <div class="space-y-1">
                            <p class="text-xs font-bold text-gray-400 uppercase">Lokasi Pengambilan</p>
                            <p class="font-bold text-gray-900">{{ $item->lokasi->nama_lokasi ?? '-' }}</p>
                        </div>
                        <div class="space-y-1">
                            <p class="text-xs font-bold text-gray-400 uppercase">Jadwal Pengambilan</p>
                            <p class="font-bold text-gray-900">
                                {{ $item->tanggal_janji_temu ? \Carbon\Carbon::parse($item->tanggal_janji_temu)->format('d M Y') : '-' }} • 
                                {{ $item->waktu_janji_temu ? \Carbon\Carbon::parse($item->waktu_janji_temu)->format('H:i') : '-' }}
                            </p>
                        </div>
                        @endif
                    </div>
                    ...
                    <div class="flex items-center gap-3">
                        <a href="{{ route('nasabah.tabungan.detail-pengajuan-tarik', $item->id) }}" 
                            class="flex-1 py-3.5 bg-linear-to-r from-[#8b6f2f] to-[#d4af37] text-white rounded-2xl font-bold text-sm hover:translate-y-[-2px] hover:shadow-lg transition-all duration-300 flex items-center justify-center gap-2">
                        ...
                        @if($item->status == '1')
                        <button type="button" 
                            onclick="openCancelModal('{{ route('nasabah.tabungan.cancel-pengajuan-tarik', $item->id) }}', 'Pengajuan Penarikan #{{ $item->id }}')"
            ...
            <div class="bg-white rounded-3xl shadow-sm border border-gray-100 p-8 sm:p-16 text-center">
                ...
                <p class="text-gray-500 mb-8 max-w-sm mx-auto font-medium">
                    Kami tidak menemukan data pengajuan penarikan untuk kategori 
                    <span class="text-[#8b6f2f] font-bold">"{{ $filters[$currentStatus ?? null]['label'] }}"</span>.
                </p>
                @if(!($currentStatus ?? null))
                <a href="{{ route('nasabah.tabungan.penarikan') }}" 
                    class="inline-flex items-center gap-2 px-8 py-3.5 bg-linear-to-r from-[#8b6f2f] to-[#d4af37] text-white rounded-2xl font-bold hover:translate-y-[-2px] hover:shadow-lg transition-all">
