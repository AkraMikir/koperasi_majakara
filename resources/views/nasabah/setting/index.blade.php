...
<div class="w-full pb-6">
    ...
    <div class="mx-4 mb-6">
        <div class="bg-white rounded-2xl shadow-lg p-2 border border-gray-100">
            <div class="flex gap-2">
                <button onclick="switchTab('password')" id="tab-password" class="flex-1 py-3 px-4 rounded-xl font-semibold transition-all bg-linear-to-r from-[#674c1d] to-[#8b6f2f] text-white shadow-md">
    ...
    <div id="content-password" class="mx-4 space-y-6">
        ...
        <div class="bg-white rounded-2xl shadow-lg p-6 border border-gray-100">
            ...
            <form method="POST" action="{{ route('nasabah.setting.change-password') }}" class="space-y-4">
                ...
                <button type="submit" class="w-full py-3 bg-linear-to-r from-[#674c1d] to-[#8b6f2f] text-white rounded-xl font-bold shadow-lg hover:shadow-xl transition-all transform hover:scale-[1.01]">
                    Ubah Password
                </button>
            </form>
        </div>
        ...
        <div class="bg-white rounded-2xl shadow-lg p-6 border border-gray-100">
            ...
            <div id="step-send-otp-pwd" class="space-y-4">
                <button type="button" onclick="sendOtpPasswordReset()" id="btn-send-otp-pwd" class="w-full py-3 bg-linear-to-r from-red-600 to-red-700 text-white rounded-xl font-bold shadow-lg hover:shadow-xl transition-all transform hover:scale-[1.01]">
                    <svg class="w-5 h-5 inline mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <form id="form-reset-password" class="hidden space-y-4 mt-4">
                ...
                <div class="flex gap-3">
                    ...
                    <button type="button" onclick="submitResetPassword()" class="flex-1 py-3 bg-linear-to-r from-red-600 to-red-700 text-white rounded-xl font-bold shadow-lg hover:shadow-xl transition-all">
                        Reset Password
                    </button>
                </div>
            </form>
    ...
    <div id="content-pin" class="mx-4 space-y-6 hidden">
        ...
        <!-- Info Lupa PIN -->
        <div class="bg-linear-to-br from-orange-50 to-red-50 rounded-2xl shadow-lg p-6 border-2 border-orange-200">
            <div class="flex items-start gap-4">
                ...
                <div class="flex-1">
                    ...
                    <a href="https://wa.me/628139552626?text=Halo%20Admin%20Koperasi%2C%0A%0ASaya%20lupa%20PIN%20transaksi%20saya.%0A%0ANama%3A%20{{ urlencode($user->nama) }}%0AEmail%3A%20{{ urlencode($user->email) }}%0ANo%20HP%3A%20{{ urlencode($user->nomor_hp) }}%0A%0AMohon%20bantuannya%20untuk%20reset%20PIN.%0ATerima%20kasih." 
                        target="_blank"
                        class="inline-flex items-center gap-2 px-6 py-3 bg-linear-to-r from-green-600 to-green-700 text-white rounded-xl font-bold shadow-lg hover:shadow-xl transition-all transform hover:scale-[1.02]">
...
<script>
    ...
    function switchTab(tab) {
        ...
        tabs.forEach(t => {
            ...
            if (t === tab) {
                btn.className = 'flex-1 py-3 px-4 rounded-xl font-semibold transition-all bg-linear-to-r from-[#674c1d] to-[#8b6f2f] text-white shadow-md';
