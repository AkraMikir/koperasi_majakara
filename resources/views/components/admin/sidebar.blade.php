@php
    ...
    $isActive = function($route) use ($currentRoute) {
        return str_starts_with($currentRoute, $route) ? 'bg-gradient-to-r from-[#674c1d] to-[#8b6f2f] text-white shadow-lg' : 'text-gray-700 hover:bg-gray-100';
    };
@endphp

<aside id="adminSidebar" class="fixed lg:static inset-y-0 left-0 z-50 w-64 bg-white shadow-xl transform transition-transform duration-300 ease-in-out lg:translate-x-0 -translate-x-full">
    <div class="flex flex-col h-full">
        <!-- Logo Section -->
        <div class="flex items-center justify-center h-20 px-6 border-b border-gray-200 bg-gradient-to-r from-[#674c1d] to-[#8b6f2f]">
        ...
        <nav class="flex-1 px-4 py-6 space-y-2 overflow-y-auto">
            ...
            <a href="{{ route('admin.pinjaman.index') }}" 
                class="flex items-center px-4 py-3 rounded-xl transition-all duration-200 {{ $isActive('admin.pinjaman') }}">
                <div class="w-10 h-10 flex items-center justify-center mr-3">
                    <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
